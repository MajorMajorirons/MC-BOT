...
public class ZoneCommand implements CommandExecutor {
    ...
    private void cmdClaim(CommandSender sender, String[] args) {
        ...
        int[] c = parseCoords(player, args, 2);
        if (c == null) return;
        ...
        player.sendMessage(GR + "爆発保護: /zone explosion " + name + " on");
    }
    ...
    private void cmdBuy(CommandSender sender, String[] args) {
        ...
        if (!confirmed) {
            // 確認待ちに登録（30秒有効）
            long expire = System.currentTimeMillis() + 30_000;
            pendingBuy.put(player.getUniqueId(), new String[]{zoneName, String.valueOf(expire)});
        ...
        if (!economy.has(player, price)) {
            player.sendMessage(E + "残高が不足しています。必要: " + economy.format(price) +
                " / 所持: " + economy.format(economy.getBalance(player)));
            return;
        }
        ...
        if (!withdraw.transactionSuccess()) {
            player.sendMessage(E + "決済に失敗しました: " + withdraw.errorMessage); return;
        }
        if (sellerUuid != null) {
            ...
            if (onlineSeller != null) {
                onlineSeller.sendMessage(GO + "🏠 ゾーン '" + zone.getName() + "' が " +
                    player.getName() + " に " + economy.format(price) + " で売れました！");
            }
        }
    ...
    private void cmdForSale(CommandSender sender, String[] args) {
        ...
        for (Zone z : forSale) {
            ...
            sender.sendMessage(C + z.getName() + GR + " | 所有者: " + z.getOwnerName() +
                " | " + GO + price + GR + " | /zone buy " + z.getName());
        }
    }
    ...
    private void cmdInfo(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2) {
            ...
            sender.sendMessage(E + "使い方: /zone info <名前>"); return;
        }

        String saleStr = zone.isForSale() && economy != null
            ? G + "販売中 " + GO + economy.format(zone.getSellPrice())
            : GR + "非売品";
    ...
    private void cmdAdminZone(CommandSender sender, String[] args) {
        ...
        int[] c = parseCoords(sender, args, 2);
        if (c == null) return;
        String world = (sender instanceof Player p) ? p.getWorld().getName() : "world";
        storage.addZone(new Zone(name, world, c[0], c[1], c[2], c[3], c[4], c[5], null, "ADMIN", true, System.currentTimeMillis()));
    ...
    private int[] parseCoords(CommandSender sender, String[] args, int offset) {
        try {
            ...
            sender.sendMessage(E + "座標は整数で入力してください。");
            return null;
        }
    }
